<template>
  <div class="flex flex-col lg:flex-row flex-no-wrap w-full">
    ...
    <JobRelatedSidebar class="hidden lg:block" v-if="thisjob" :jobs="relatedJobs" :job="thisjob" />
    <JobResultLayout>
      <template #content>
        <JobListing v-if="thisjob" :job="thisjob" :key="thisjob.id" element="div">
          <template #others>
            ...
            <div class="px-8 py-4">
              ...
              <ul class="list-disc pl-8">
                <li
                  v-for="preferredQualification in thisjob.preferredQualifications"
...
watchEffect(async () => {
  ...
  relatedJobs.value = ALL_JOBS.value.filter(
    (job: Job) => job.organization === thisjob?.value?.organization
  )
})
</script>
